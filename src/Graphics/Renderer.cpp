...
void Renderer::DrawPass(const DrawSceneDesc& desc) const
{
...

	{
...

		{
			...
			if (desc.drawSprites)
			{
				...
				Mesh* spritePlane;
				registry.Each<const SpriteMesh>(
				    [&spritePlane](const SpriteMesh& spriteMesh) { spritePlane = spriteMesh.plane.get(); });
				...
				registry.Each<const Sprite, const Transform>(
				    [&spritePlane, &spriteShader, &desc](const Sprite& sprite, const Transform& transform) {
					    ...
					    spritePlane->GetVertexBuffer().Bind();
...

	{
		...
		{
			Mesh* debugCross;
			const glm::mat4* debugCrossPose;

			auto& registry = Locator::entitiesRegistry::value();
			registry.Each<const ecs::components::DebugCross>(
			    [&debugCross, &debugCrossPose](const ecs::components::DebugCross& ent) {
				    debugCross = ent.debugCross.get();
				    debugCrossPose = &(ent.debugCrossPose);
			    });

			bgfx::setTransform(glm::value_ptr(*debugCrossPose));
			debugCross->GetVertexBuffer().Bind();
			bgfx::setState(k_BgfxDefaultStateInvertedZ | BGFX_STATE_PT_LINES);
			bgfx::submit(static_cast<bgfx::ViewId>(desc.viewId), debugShader->GetRawHandle());
		}
	}
